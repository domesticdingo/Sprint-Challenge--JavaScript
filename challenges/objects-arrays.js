...
const universities = graduates.map((x) => x.university);
...
const unisWithUni = universities.filter((x) => x.includes('Uni'));
...
*/
const regularAnimalNames = zooAnimals.map((x) => x.animal_name);

const lowCaseAnimalNames = regularAnimalNames.map(function(regularAnimalNames) {
  return regularAnimalNames.toLowerCase();
});
...
*/
const allAnimals = zooAnimals.map((x) => x.population);

const populationTotal = allAnimals.reduce((acc, cv) => {
  return acc + cv;
},0);
